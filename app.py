...
# st.markdown("<h4 style='position: fixed; top: 0px; z-index: 99; padding-top: 1rem; text-align: center;padding-top: 0rem;'>테스트</h4>", unsafe_allow_html=True)
    ...
    # Place for plots depending on how many models
        ...
        for plot_num, model_type in enumerate(selected_models):
            with plots[plot_num]:
                ...
                try:
                    # Format the question
                    question_to_ask = format_question(primer1, primer2, prompt, model_type)
                    # Run the question
                    answer=""
                    answer = run_request(question_to_ask, available_models[model_type], key=client)
                    # the answer is the completed Python script so add to the beginning of the script to it.
...
run = client.beta.threads.runs.create(
    ...
    instructions="당신은 조만간 외부감사인에게 감사를 받게 될것입니다. 사용자가 업로드한 파일에서만 찾은 답변을 해주십시오. 만약, 업로드한 파일내에서 정보를 찾을 수 없으면 거기에서 답변을 멈추세요. 목록이 5줄 이상이 되면 목록 제목이 가로로 된 표로 작성해주십시오."
)
